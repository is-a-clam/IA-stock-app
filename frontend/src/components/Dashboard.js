...
class Dashboard extends React.Component {
  ...
  componentDidMount() {
    axios.get('api/user-dash/').then(res => {
  ...
  amountOnChange(e, { value: amount }) {
    ...
    if (amount <= 0) {
      this.setState({
        transactionAmountErr: 'Please input a positive number',
      })
      return
    }
    if (this.state.transactionModalMode === 'Sell') {
      if (amount > this.state.dash[this.state.transactionModalStock].amount) {
        this.setState({
          transactionAmountErr: 'You cannot sell more than you currently have',
        })
        return
      }
    }
  ...
  priceOnChange(e, { value: price }) {
    ...
    if (price <= 0) {
      this.setState({
        transactionPriceErr: 'Please input a positive number',
      })
      return
    }
    this.setState({
  ...
  render() {
    return (
      ...
      >
            ...
            return (
              <List.Item style={{ marginBottom: '10px' }}>
                <Grid style={{ marginLeft: '0px' }}>
                  ...
                  >
                    <Icon link name='close' onClick={() => this.handleRemoveFromDashboard(symbol)} />
                  </Grid.Column>
                </Grid>
              </List.Item>
            )
        ...
        >
          <Modal.Header>{this.state.transactionModalMode}</Modal.Header>
          <Modal.Content>
            <p>Please enter amount and price of {this.state.transactionModalStock} stock</p>
            <br></br>
            <Form>
              <Form.Group widths='equal'>
                <Form.Input
                  ...
                  error={
                    this.state.transactionAmountErr && {
                ...
                <Form.Input
                  ...
                  error={
                    this.state.transactionPriceErr && {
